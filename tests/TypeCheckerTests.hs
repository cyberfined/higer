...
caseLVal = mkTestLabel "type checking lvals"
    [ assertTypeCheck "let var v := 5 in v + 5 end" $ mkLet
        [ mkTypedVar "v" TInt Remaining (mkIntLit 5) ] (mkLVal (Var "v") $+ mkIntLit 5)
    , assertTypeCheck "let var v: int := 5 in v + 5 end" $ mkLet
        [ mkTypedVar "v" TInt Remaining (mkIntLit 5) ] (mkLVal (Var "v") $+ mkIntLit 5)
    , assertTypeCheck "let var v := \"abcd\" in v end" $ mkLet
        [ mkTypedVar "v" TString Remaining (mkStrLit "abcd") ] (mkLVal (Var "v"))
    , assertTypeCheck "let var v : string := \"abcd\" in v end" $ mkLet
        [ mkTypedVar "v" TString Remaining (mkStrLit "abcd") ] (mkLVal (Var "v"))
    , assertTypeCheckML
        ...
        [ mkTypedType "intarr" (TArray TInt)
        , mkTypedVar "arr" (TArray TInt) Remaining
            (mkArray "intarr" (mkIntLit 12) (mkIntLit 0))
    ...
    , assertTypeCheckML
        ...
        [ mkTypedType "rec" (TRecord "rec" [RecordField "x" TInt])
        , mkTypedVar "v" (TRecord "rec" [RecordField "x" TInt]) Remaining
            (mkRecord "rec" [RecordField "x" (mkIntLit 5)])
    ...
    , assertTypeCheckML
        ...
        [ mkTypedType "rec"
            (TRecord "rec" [RecordField "x" TInt, RecordField "y" TString])
        , mkTypedVar "v" (TRecord "rec" [RecordField "x" TInt, RecordField "y" TString])
            Remaining
            (mkRecord "rec"
                [RecordField "x" (mkIntLit 5), RecordField "y" (mkStrLit "abcd")])
        ] $ mkSeq
    ...
    , assertTypeCheckML
        ...
        [ mkTypedType "intlist"
            (TRecord "intlist" [RecordField "val" TInt, RecordField "rest" TSelf])
        , mkTypedVar "lst"
            (TRecord "intlist" [RecordField "val" TInt, RecordField "rest" TSelf])
            Remaining
            (mkRecord "intlist" [ RecordField "val" (mkIntLit 4)
                                , RecordField "rest" $
                                    mkRecord "intlist" [ RecordField "val" (mkIntLit 5)
                                                       , RecordField "rest" mkNil
                                                       ]
                                ]
            )
...
caseAssignment = mkTestLabel "type checking assignment"
    [ assertTypeCheckML
        ...
        [ mkTypedType "list" (TRecord "list" [RecordField "x" TInt])
        , mkTypedVar "v" (TRecord "list" [RecordField "x" TInt]) Remaining mkNil
        ] $ mkLVal (Var "v")
    , assertTypeCheckML
        ...
        [ mkTypedType "list"
            (TRecord "list" [RecordField "x" TInt, RecordField "next" TSelf])
        , mkTypedVar "v"
            (TRecord "list" [RecordField "x" TInt, RecordField "next" TSelf])
            Remaining
            (mkRecord "list" [RecordField "x" (mkIntLit 1), RecordField "next" mkNil])
...
caseIf = mkTestLabel "type checking if expression"
    ...
    , assertTypeCheckML
        ...
        , mkTypedVar "v" (TRecord "rec" [RecordField "x" TInt]) Remaining $
            mkIf [ (mkIntLit 2) $> (mkIntLit 5)
                 , mkNil
                 , mkRecord "rec" [RecordField "x" (mkIntLit 1)]
                 ]
        ] $ mkLVal (Var "v")
    , assertTypeCheckML
        [ "let type rec = {x:int}"
...
caseCall = mkTestLabel "type checking function calls"
    [ assertTypeCheckML
        ...
        [ mkTypedFun "f" [ TypedFunArg "x" TInt Remaining
                         , TypedFunArg "y" TString Remaining
                         ] TInt (mkLVal (Var "x"))
        ]
    ...
    , assertTypeCheckML
        ...
        [ mkTypedFun "f" [] TUnit (mkCall "print" [mkStrLit "abcd"]) ] $ mkCall "f" []
    , assertTypeCheckML
        [ "let function fib(n:int):int = if n <= 2 then 1 else fib(n-1) + fib(n-2)"
        , "in fib(55)"
        , "end"
        ] $ mkLet
        [ mkTypedFun "fib" [TypedFunArg "n" TInt Remaining] TInt $ mkIf
    ...
    , assertTypeCheckML
        ...
        [ mkTypedFun "is_even" [TypedFunArg "n" TInt Remaining] TInt $
        ...
        , mkTypedFun "is_odd" [TypedFunArg "n" TInt Remaining] TInt $
            mkIf [ mkLVal (Var "n") $< mkIntLit 0
    ...
    , assertTypeCheckML
        ...
        [ mkTypedType "rec" (TRecord "rec" [RecordField "x" TInt])
        , mkTypedFun "f" [] (TRecord "rec" [RecordField "x" TInt])
            (mkRecord "rec" [RecordField "x" (mkIntLit 1)])
        , mkTypedVar "v" (TRecord "rec" [RecordField "x" TInt]) Remaining (mkCall "f" [])
        ] $ mkLVal (Var "v")
    , assertTypeCheckML
        [ "let type rec = {x:int}"
        , "function f(): rec = nil"
        ...
        [ mkTypedType "rec" (TRecord "rec" [RecordField "x" TInt])
        , mkTypedFun "f" [] (TRecord "rec" [RecordField "x" TInt]) mkNil
        , mkTypedVar "v" (TRecord "rec" [RecordField "x" TInt]) Remaining (mkCall "f" [])
        ] $ mkLVal (Var "v")
    , assertTypeCheckML
        [ "let type rec = {x:int}"
        ...
        [ mkTypedType "rec" (TRecord "rec" [RecordField "x" TInt])
        , mkTypedFun "f"
            [TypedFunArg "y" (TRecord "rec" [RecordField "x" TInt]) Remaining] TInt
...
caseWhileFor = mkTestLabel "while, for type checking"
    [ assertTypeCheckML
        ...
        [ mkTypedVar "i" TInt Remaining (mkIntLit 49) ] $
    ...
    , assertTypeCheck "for i := 49 to 54 do print(chr(i))" $
        mkFor "i" Remaining (mkIntLit 49) (mkIntLit 54)
            (mkCall "print" [mkCall "chr" [mkLVal (Var "i")]])
